    ...
    view! {
        <div class=tw_merge!("rounded-lg border p-3", class.get())>
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold">{move || format!("{} {}", month_name(), year())}</h2>
                <div class="flex items-center gap-2">
                    <Button variant={Signal::from(ButtonVariant::Outline)} size={Signal::from(ButtonSize::Icon)} onclick=Callback::new(prev_month)>
                        <Icon icon=icondata::LuChevronLeft style="h-4 w-4" />
                    </Button>
                    <Button variant={Signal::from(ButtonVariant::Outline)} size={Signal::from(ButtonSize::Icon)} onclick=Callback::new(next_month)>
                        <Icon icon=icondata::LuChevronRight style="h-4 w-4" />
                    </Button>
                </div>
            </div>
            <div class="grid grid-cols-7 gap-2 mt-4 text-center text-sm">
                {weekdays.into_iter().map(|day| {
                    view! { <div class="text-muted-foreground">{day}</div> }
                }).collect_view()}

                {(0..first_day_of_month_weekday()).map(|_| view! { <div></div> }).collect_view()}

                {(1..=days_in_month()).map(move |day| {
                    let current_date = NaiveDate::from_ymd_opt(view_date.get().year(), view_date.get().month(), day).unwrap();
